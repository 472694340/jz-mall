...
public class JwtUtil {
    ...
    /**
     *  根据用户信息生成Claims 继而生成token
     *   这里是根据username 还是 userDetails里的信息?
     *   答案是 根据userDetails里的信息,因为是登录成功后,每个用户登录成功会在上下文里存入相应的信息,不然这里的username怎么来?
     */
    //错误示范,不是要存放密码,而是要存放用户的相关信息(包括,用户名,创建时间) //        map.put(); //是否要存放密码/权限
    public String generatorTokenByClaims(UserDetails userDetails){
